...
static const u8 sText_MonsFullyHealed[] = _("Your team has been fully healed!{PAUSE_UNTIL_PRESS}");
    ...
    if (canRestore)
    {
        ...
        for (i = 0; i < gPlayerPartyCount; i++)
        {
             ...
             && GetMonData(&gPlayerParty[i], MON_DATA_HP) != 0)
            {
                ...
                SetMonData(&gPlayerParty[i], MON_DATA_HP, &maxHp);
                MonRestorePP(&gPlayerParty[i]);
                HealStatusConditions(&gPlayerParty[i], STATUS1_ANY | STATUS1_TOXIC_COUNTER, MAX_BATTLERS_COUNT);
            }
        }
        if (!gTasks[taskId].tUsingRegisteredKeyItem)
            DisplayItemMessage(taskId, FONT_NORMAL, sText_MonsFullyHealed, CloseItemMessage);
        else
            DisplayItemMessageOnField(taskId, sText_MonsFullyHealed, Task_CloseCantUseKeyItemMessage);
    }
    else
    {
        gSpecialVar_Result = 0;
...
void ItemUseOutOfBattle_PortablePC(u8 taskId)
{
    if ((gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(MAP_EVER_GRANDE_CITY_SIDNEYS_ROOM) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(MAP_EVER_GRANDE_CITY_SIDNEYS_ROOM))
     ...
     || (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(MAP_EVER_GRANDE_CITY_SPENSERS_ROOM) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(MAP_EVER_GRANDE_CITY_SPENSERS_ROOM))
     || (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(MAP_EVER_GRANDE_CITY_LUCYS_ROOM) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(MAP_EVER_GRANDE_CITY_LUCYS_ROOM))
     || (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(MAP_EVER_GRANDE_CITY_TUCKERS_ROOM) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(MAP_EVER_GRANDE_CITY_TUCKERS_ROOM))
     || (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(MAP_EVER_GRANDE_CITY_BRANDONS_ROOM) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(MAP_EVER_GRANDE_CITY_BRANDONS_ROOM))
