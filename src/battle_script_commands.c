...
static void SetNonVolatileStatus(u32 effectBattler, enum MoveEffect effect, const u8 *battleScript, enum StatusTrigger trigger)
{
    ...
    case MOVE_EFFECT_SLEEP:
        if (B_SLEEP_TURNS >= GEN_LATEST)
            gBattleMons[effectBattler].status1 |= STATUS1_SLEEP_TURN(2 + RandomWeighted(RNG_SLEEP_TURNS, 1, 2));
    ...
    case EFFECT_STEAL_ITEM:
        ...
        else
        {
            ...
            if (!(GetConfig(CONFIG_STEAL_WILD_ITEMS) >= GEN_9
             && !(gBattleTypeFlags & (BATTLE_TYPE_TRAINER | BATTLE_TYPE_PALACE))))
            {
                if (!IsOnHpThresholdActivation(GetItemHoldEffect(gLastUsedItem)))
                {
                ...
                // HP-threshold berries stay assigned so MOVEEND_ITEM_EFFECTS_ATTACKER_2 can eat them right away if their requirements are met
            }
            gEffectBattler = gBattlerTarget;
